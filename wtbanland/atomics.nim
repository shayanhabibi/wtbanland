## Minor adjustments have been made, and will continue to be made to make this
## library more suitable for algorithms that perform atomic operations on
## aligned pointers.
## 
## - rawLoad: exported proc that accesses internal value of Atomic
## - MemoryOrder Aliases: Shortened aliases for memory ordering have been defined
## - All numerical operations no longer require the type to be SomeInteger and
## can be performed on any type. User beware.
...
when defined(cpp) or defined(nimdoc):
  ...
  proc fetchAdd*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.importcpp: "#.fetch_add(@)".}
    ## Atomically adds a `value` to the atomic integer and returns the
    ## original value.

  proc fetchSub*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.importcpp: "#.fetch_sub(@)".}
    ## Atomically subtracts a `value` to the atomic integer and returns the
    ## original value.

  proc fetchAnd*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.importcpp: "#.fetch_and(@)".}
    ## Atomically replaces the atomic integer with it's bitwise AND
    ## with the specified `value` and returns the original value.

  proc fetchOr*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.importcpp: "#.fetch_or(@)".}
    ## Atomically replaces the atomic integer with it's bitwise OR
    ## with the specified `value` and returns the original value.

  proc fetchXor*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.importcpp: "#.fetch_xor(@)".}
  ...
  when defined(vcc):
    ...
    proc fetchAdd*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      var currentValue = location.load()
      while not compareExchangeWeak(location, currentValue, currentValue + value): discard
    proc fetchSub*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      fetchAdd(location, -value, order)
    proc fetchAnd*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      cast[T](interlockedAnd(addr(location.value), cast[nonAtomicType(T)](value)))
    proc fetchOr*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      cast[T](interlockedOr(addr(location.value), cast[nonAtomicType(T)](value)))
    proc fetchXor*[T](location: var Atomic[T]; value: SomeInteger; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
    ...
    proc rawLoad*[T](location: var Atomic[T]): T {.inline.} =
      when T is Trivial:
        cast[T](location.value)
      else:
        location.nonAtomicValue
...
proc atomicInc*[T](location: var Atomic[T]; value: SomeInteger = 1) {.inline.} =
  ## Atomically increments the atomic integer by some `value`.
  discard location.fetchAdd(value)

proc atomicDec*[T](location: var Atomic[T]; value: SomeInteger = 1) {.inline.} =
  ## Atomically decrements the atomic integer by some `value`.
  discard location.fetchSub(value)

proc `+=`*[T](location: var Atomic[T]; value: SomeInteger) {.inline.} =
  ## Atomically increments the atomic integer by some `value`.
  discard location.fetchAdd(value)
...
const
  moSeqCon* = moSequentiallyConsistent
  moRlx* = moRelaxed
  moRel* = moRelease
  moAcq* = moAcquire
  moAcqRel* = moAcquireRelease
  ## Shortened alias to memory orders